...
SOLO_API::SOLO_API(int *argc, char ***argv){
    if(*argc!= 2)
    {
	...
	double kp = 5.;
	double kd = 0.1;
	double iq_sat = 4.0; // check for the use of saturation current in motor
	double freq = 0.5;
	double amplitude = M_PI;
	double init_pos[N_SLAVES * 2] = {0}; // ask for what the curly brackets denote to whether a dictionary or a position??? What???
	int state = 0;
